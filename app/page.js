...
import {
  BookOpen, BarChart2, Calendar, Settings, ChevronRight, ChevronLeft, Plus, Trash2,
  RefreshCw, CheckCircle, XCircle, Eye, EyeOff, Moon, Sun, BookMarked, List,
  Flame, Star, Volume2, Info, X, Check, ChevronDown, Menu, Grid3X3,
  BookText, Play, Pause, Loader2, Search, Save, Clock, Bookmark,
  Hash, LayoutDashboard, AlertCircle, Zap,
  Lightbulb, Edit2, CheckSquare,
} from 'lucide-react';

// ─── localStorage Store ───────────────────────────────────────────────────────
const STORE = {
  get(key) { try { return JSON.parse(localStorage.getItem(key) || '[]'); } catch { return []; } },
  getOne(key, d = {}) { try { return JSON.parse(localStorage.getItem(key) || JSON.stringify(d)); } catch { return d; } },
  set(key, val) { try { localStorage.setItem(key, JSON.stringify(val)); } catch {} },
  push(key, item) { const a = this.get(key); a.push(item); this.set(key, a); return item; },
  update(key, id, up) {
    const a = this.get(key); const i = a.findIndex(x => x.id === id);
    if (i !== -1) { a[i] = { ...a[i], ...up }; this.set(key, a); return a[i]; } return null;
  },
...
function getSettings() {
  return STORE.getOne('q_settings', {
    requiredChecks: 3, darkMode: false, defaultTafsir: '91',
    quranFontSize: 'md', reciter: 'ar.alafasy', tajweedColors: true, palette: 'teal',
  });
}
function saveSettings(s) { STORE.set('q_settings', s); }
function getBookmarks() { return STORE.get('q_bookmarks'); }
...
function getLastPosition() { return STORE.getOne('q_last_pos', { page: 1, verseKey: null }); }
function saveLastPosition(page, verseKey) { STORE.set('q_last_pos', { page, verseKey, date: new Date().toISOString() }); }
function getStreak() { return STORE.getOne('q_streak', { current: 0, best: 0, lastDate: null }); }
...
const PALETTES = {
  teal: {
    name: 'فيروزي', light: {
      bg: '#F0FAF6', bg2: '#E1F5EE', bg3: '#C8EDDF',
      surface: '#FAFFFE', surface2: '#F0FAF6', border: 'rgba(15,110,86,0.08)',
      border2: 'rgba(15,110,86,0.15)', border3: 'rgba(15,110,86,0.25)',
      green: '#0F6E56', green2: '#1D9E75', green3: '#5DCAA5', greenGlow: 'rgba(15,110,86,0.2)',
      greenBg: 'rgba(15,110,86,0.07)', greenBorder: 'rgba(15,110,86,0.18)',
    },
    dark: {
      bg: '#04140E', bg2: '#071C14', bg3: '#0A2419',
      surface: '#061510', surface2: '#091A13', border: 'rgba(93,202,165,0.08)',
      border2: 'rgba(93,202,165,0.15)', border3: 'rgba(93,202,165,0.25)',
      green: '#5DCAA5', green2: '#9FE1CB', green3: '#C8EDDF', greenGlow: 'rgba(93,202,165,0.22)',
      greenBg: 'rgba(93,202,165,0.09)', greenBorder: 'rgba(93,202,165,0.2)',
    }
  },
  emerald: {
    name: 'زمردي', light: {
      bg: '#F0FAF2', bg2: '#DCF5E2', bg3: '#C0ECCC',
      surface: '#F8FFF9', surface2: '#EEF9F1', border: 'rgba(11,68,27,0.08)',
      border2: 'rgba(11,68,27,0.15)', border3: 'rgba(11,68,27,0.25)',
      green: '#0D5236', green2: '#1A7A50', green3: '#3BAF74', greenGlow: 'rgba(13,82,54,0.2)',
      greenBg: 'rgba(13,82,54,0.07)', greenBorder: 'rgba(13,82,54,0.18)',
    },
    dark: {
      bg: '#030E07', bg2: '#061209', bg3: '#091A0D',
      surface: '#050F08', surface2: '#07140A', border: 'rgba(59,175,116,0.08)',
      border2: 'rgba(59,175,116,0.15)', border3: 'rgba(59,175,116,0.25)',
      green: '#3BAF74', green2: '#60CF96', green3: '#97E8BE', greenGlow: 'rgba(59,175,116,0.22)',
      greenBg: 'rgba(59,175,116,0.09)', greenBorder: 'rgba(59,175,116,0.2)',
    }
  },
  sapphire: {
    name: 'ياقوتي', light: {
      bg: '#EEF4FD', bg2: '#DCEBFA', bg3: '#C3D9F7',
      surface: '#F8FBFF', surface2: '#EEF4FD', border: 'rgba(24,95,165,0.08)',
      border2: 'rgba(24,95,165,0.15)', border3: 'rgba(24,95,165,0.25)',
      green: '#185FA5', green2: '#378ADD', green3: '#85B7EB', greenGlow: 'rgba(24,95,165,0.2)',
      greenBg: 'rgba(24,95,165,0.07)', greenBorder: 'rgba(24,95,165,0.18)',
    },
    dark: {
      bg: '#020A14', bg2: '#040D1A', bg3: '#071220',
      surface: '#030C17', surface2: '#05101C', border: 'rgba(133,183,235,0.08)',
      border2: 'rgba(133,183,235,0.15)', border3: 'rgba(133,183,235,0.25)',
      green: '#85B7EB', green2: '#B5D4F4', green3: '#D6E9FA', greenGlow: 'rgba(133,183,235,0.22)',
      greenBg: 'rgba(133,183,235,0.09)', greenBorder: 'rgba(133,183,235,0.2)',
    }
  },
  amber: {
    name: 'عنبري', light: {
      bg: '#FDF5E8', bg2: '#FAE9CC', bg3: '#F5D8A0',
      surface: '#FFFDF7', surface2: '#FDF5E8', border: 'rgba(133,79,11,0.08)',
      border2: 'rgba(133,79,11,0.15)', border3: 'rgba(133,79,11,0.25)',
      green: '#854F0B', green2: '#BA7517', green3: '#EF9F27', greenGlow: 'rgba(133,79,11,0.2)',
      greenBg: 'rgba(133,79,11,0.07)', greenBorder: 'rgba(133,79,11,0.18)',
    },
    dark: {
      bg: '#130C02', bg2: '#1C1203', bg3: '#261805',
      surface: '#100A01', surface2: '#171003', border: 'rgba(239,159,39,0.08)',
      border2: 'rgba(239,159,39,0.15)', border3: 'rgba(239,159,39,0.25)',
      green: '#EF9F27', green2: '#FAC775', green3: '#FAEEDA', greenGlow: 'rgba(239,159,39,0.22)',
      greenBg: 'rgba(239,159,39,0.09)', greenBorder: 'rgba(239,159,39,0.2)',
    }
  },
  rose: {
    name: 'وردي', light: {
      bg: '#FDF0F4', bg2: '#FAD9E6', bg3: '#F5BAD0',
      surface: '#FFFAFC', surface2: '#FDF0F4', border: 'rgba(153,53,86,0.08)',
      border2: 'rgba(153,53,86,0.15)', border3: 'rgba(153,53,86,0.25)',
      green: '#993556', green2: '#D4537E', green3: '#ED93B1', greenGlow: 'rgba(153,53,86,0.2)',
      greenBg: 'rgba(153,53,86,0.07)', greenBorder: 'rgba(153,53,86,0.18)',
    },
    dark: {
      bg: '#140508', bg2: '#1C070D', bg3: '#260A12',
      surface: '#100304', surface2: '#180609', border: 'rgba(237,147,177,0.08)',
      border2: 'rgba(237,147,177,0.15)', border3: 'rgba(237,147,177,0.25)',
      green: '#ED93B1', green2: '#F4C0D1', green3: '#FAE9F1', greenGlow: 'rgba(237,147,177,0.22)',
      greenBg: 'rgba(237,147,177,0.09)', greenBorder: 'rgba(237,147,177,0.2)',
    }
  },
  purple: {
    name: 'بنفسجي', light: {
      bg: '#F1EFFD', bg2: '#E1DEFA', bg3: '#CABDF6',
      surface: '#FAFAFE', surface2: '#F1EFFD', border: 'rgba(83,74,183,0.08)',
      border2: 'rgba(83,74,183,0.15)', border3: 'rgba(83,74,183,0.25)',
      green: '#534AB7', green2: '#7F77DD', green3: '#AFA9EC', greenGlow: 'rgba(83,74,183,0.2)',
      greenBg: 'rgba(83,74,183,0.07)', greenBorder: 'rgba(83,74,183,0.18)',
    },
    dark: {
      bg: '#060513', bg2: '#0A0820', bg3: '#0D0B2A',
      surface: '#04030F', surface2: '#080618', border: 'rgba(175,169,236,0.08)',
      border2: 'rgba(175,169,236,0.15)', border3: 'rgba(175,169,236,0.25)',
      green: '#AFA9EC', green2: '#CECBF6', green3: '#EEEDFE', greenGlow: 'rgba(175,169,236,0.22)',
      greenBg: 'rgba(175,169,236,0.09)', greenBorder: 'rgba(175,169,236,0.2)',
    }
  },
  slate: {
    name: 'رمادي', light: {
      bg: '#F2F0EA', bg2: '#E6E4DC', bg3: '#D4D2CA',
      surface: '#FEFEFE', surface2: '#F2F0EA', border: 'rgba(95,94,90,0.08)',
      border2: 'rgba(95,94,90,0.15)', border3: 'rgba(95,94,90,0.25)',
      green: '#5F5E5A', green2: '#888780', green3: '#B4B2A9', greenGlow: 'rgba(95,94,90,0.2)',
      greenBg: 'rgba(95,94,90,0.07)', greenBorder: 'rgba(95,94,90,0.18)',
    },
    dark: {
      bg: '#0D0D0B', bg2: '#141412', bg3: '#1C1C19',
      surface: '#0A0A09', surface2: '#111110', border: 'rgba(180,178,169,0.08)',
      border2: 'rgba(180,178,169,0.15)', border3: 'rgba(180,178,169,0.25)',
      green: '#B4B2A9', green2: '#D3D1C7', green3: '#F1EFE8', greenGlow: 'rgba(180,178,169,0.22)',
      greenBg: 'rgba(180,178,169,0.09)', greenBorder: 'rgba(180,178,169,0.2)',
    }
  },
  coral: {
    name: 'مرجاني', light: {
      bg: '#FDF0EB', bg2: '#FAD8CC', bg3: '#F5B9A4',
      surface: '#FFFAF8', surface2: '#FDF0EB', border: 'rgba(153,60,29,0.08)',
      border2: 'rgba(153,60,29,0.15)', border3: 'rgba(153,60,29,0.25)',
      green: '#993C1D', green2: '#D85A30', green3: '#F0997B', greenGlow: 'rgba(153,60,29,0.2)',
      greenBg: 'rgba(153,60,29,0.07)', greenBorder: 'rgba(153,60,29,0.18)',
    },
    dark: {
      bg: '#130601', bg2: '#1C0A02', bg3: '#260E03',
      surface: '#100401', surface2: '#170803', border: 'rgba(240,153,123,0.08)',
      border2: 'rgba(240,153,123,0.15)', border3: 'rgba(240,153,123,0.25)',
      green: '#F0997B', green2: '#F5C4B3', green3: '#FAE9E4', greenGlow: 'rgba(240,153,123,0.22)',
      greenBg: 'rgba(240,153,123,0.09)', greenBorder: 'rgba(240,153,123,0.2)',
    }
  },
};
...
async function fetchTafsir(verseKey, tafsirId = '91') {
  ...
  } catch (e) { console.warn('Tafsir fetch error:', e); return null; }
}
...
const TANWIN=/[\u064B\u064C\u064D]/,SUKUN=/\u0652/,SHADDA=/\u0651/,QALQALA_LETTERS=/[قطبجد]/,IDGHAM_LETTERS=/[يرملون]/,IKHFA_LETTERS=/[تثجدذزسشصضطظفقك]/,IQLAB_LETTER=/[ب]/,IDHHAR_LETTERS=/[حخعغهء]/,SUN_LETTERS=/[تثدذرزسشصضطظلن]/;
function isArabicDiacritic(ch){const c=ch.charCodeAt(0);return(c>=0x064B&&c<=0x065F)||c===0x0670||c===0x0671;}
function isArabicLetter(ch){const c=ch.charCodeAt(0);return(c>=0x0600&&c<=0x06FF)||(c>=0xFB50&&c<=0xFDFF)||(c>=0xFE70&&c<=0xFEFF);}
function getPrevLetterCluster(chars,idx){let i=idx-1;while(i>=0&&isArabicDiacritic(chars[i]))i--;if(i<0||!isArabicLetter(chars[i]))return null;let cl=chars[i],j=i+1;while(j<idx&&isArabicDiacritic(chars[j])){cl+=chars[j];j++;}return cl;}
function getPrevTwoLetters(chars,idx){let l='',i=idx-1,c=0;while(i>=0&&c<2){if(!isArabicDiacritic(chars[i])&&isArabicLetter(chars[i])){l=chars[i]+l;c++;}i--;}return l;}
function isMaddLetter(chars,idx,base,diac){if(base==='ا'||base==='آ'){const p=getPrevLetterCluster(chars,idx);if(p&&/[\u064E]/.test(p))return true;}if(base==='و'&&SUKUN.test(diac)){const p=getPrevLetterCluster(chars,idx);if(p&&/[\u064F]/.test(p))return true;}if(base==='ي'&&SUKUN.test(diac)){const p=getPrevLetterCluster(chars,idx);if(p&&/[\u0650]/.test(p))return true;}return false;}
function getTajweedColor(chars,idx,cluster){const base=cluster[0],diac=cluster.slice(1);if(SHADDA.test(diac)){if(/[منيو]/.test(base))return TAJWEED_COLORS.ghunna;return TAJWEED_COLORS.idgham;}if(QALQALA_LETTERS.test(base)&&SUKUN.test(diac))return TAJWEED_COLORS.qalqala;const pc=getPrevLetterCluster(chars,idx);if(pc){const pb=pc[0],pd=pc.slice(1),nk=pb==='ن'&&SUKUN.test(pd),tw=TANWIN.test(pd);if(nk||tw){if(IQLAB_LETTER.test(base))return TAJWEED_COLORS.iqlab;if(IDGHAM_LETTERS.test(base))return TAJWEED_COLORS.idgham;if(IKHFA_LETTERS.test(base))return TAJWEED_COLORS.ikhfa;if(IDHHAR_LETTERS.test(base))return TAJWEED_COLORS.idhhar;}}const pt=getPrevTwoLetters(chars,idx);if(pt==='ال'&&SUN_LETTERS.test(base)&&SHADDA.test(diac))return TAJWEED_COLORS.madd;if(isMaddLetter(chars,idx,base,diac))return TAJWEED_COLORS.madd;if(TANWIN.test(diac))return TAJWEED_COLORS.ghunna;return null;}
function applyTajweed(text){if(!text)return[];const chars=[...text],segs=[];let i=0;while(i<chars.length){const ch=chars[i];let cl=ch,j=i+1;while(j<chars.length&&isArabicDiacritic(chars[j])){cl+=chars[j];j++;}const color=getTajweedColor(chars,i,cl);if(color&&segs.length>0&&segs[segs.length-1].color===color)segs[segs.length-1].text+=cl;else if(!color&&segs.length>0&&!segs[segs.length-1].color)segs[segs.length-1].text+=cl;else segs.push({text:cl,color});i=j;}return segs;}
function TajweedText({text}){const segs=applyTajweed(text);return(<span>{segs.map((s,i)=>s.color?<span key={i} style={{color:s.color}}>{s.text}</span>:<span key={i}>{s.text}</span>)}</span>);}
...
export default function App() {
  ...
  const playVerseAudio = useCallback(async (surah, ayah, verseKey, verseText, autoNext = false) => {
    ...
    const tryPlay = (url, isFb = false) => new Promise((res, rej) => {
      ...
      audio.play().then(() => {
        if (cancelledRef.current) { audio.pause(); rej('cancelled'); return; }
        setAudioState(s => ({ ...s, playing: true, loading: false })); res();
      }).catch(e => {
        if (cancelledRef.current) { rej('cancelled'); return; }
        if (!isFb) { audio.onerror = null; audio.onloadedmetadata = null; audio.ontimeupdate = null; audio.onended = null; tryPlay(fallback, true).then(res).catch(rej); }
    ...
    try { await tryPlay(primary, false); }
    catch (e) { if (e !== 'cancelled') { setAudioState(s => ({ ...s, loading: false, playing: false })); } }
  }, [settings.reciter]);

  const playAudio = useCallback((surah, ayah, verseKey, verseText) => {
  ...
  const togglePlayPause = useCallback(() => {
    ...
    if (audioRef.current.paused) {
      audioRef.current.play().then(() => { if (!cancelledRef.current) setAudioState(s => ({ ...s, playing: true })); }).catch(() => {});
  ...
  const renderScreen = () => {
    if (screen === 'review') return (
      <ReviewPage from={reviewRange.from} to={reviewRange.to} taskId={reviewTaskId} settings={settings}
        onFinish={() => finishReview(reviewTaskId)}
        onPlayAudio={playAudio} playingKey={audioState.verseKey} hasPlayer={hasPlayer}
        onVersesLoaded={setAllVerses} />
    );
      ...
      <div className="app-shell">
        <aside className={`sidebar ${sidebarOpen ? 'open' : ''}`}>
          <div className="sidebar-brand">
            <div className="sidebar-logo"><BookOpen size={19} color="white" /></div>
            <div>
          ...
          <header className="top-bar">
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <span style={{ fontWeight: 800, fontSize: 15.5, color: 'var(--ink)' }}>
                {screen === 'review' ? 'مراجعة' : screen === 'fix' ? 'تصحيح' : navItems.find(n => n.id === navTab)?.label || 'مراجع'}
              </span>
            </div>
    ...
    <div className="audio-player">
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: 10.5, fontWeight: 700, color: 'var(--gold2)', marginBottom: 2 }}>الآية {verseKey}</div>
          {verseText && (
            <div className="quran-text" style={{ fontSize: 13, lineHeight: 1.6, color: 'var(--ink2)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
          ...
          <button className="btn btn-icon" onClick={onClose} style={{ width: 32, height: 32 }}><X size={14} /></button>
        </div>
      </div>
      <input type="range" min={0} max={duration || 100} step={0.1} value={currentTime} dir="ltr"
        onChange={e => onSeek(Number(e.target.value))}
        style={{ background: `linear-gradient(to left, var(--bg3) ${100 - progress}%, var(--green) ${100 - progress}%)` }} />
    </div>
  );
}
...
function HomeScreen({ onStartReview, settings }) {
  ...
  useEffect(() => {
    const m = getMistakes(), sess = getSessions();
    ...
    <div className="page-content">
            ...
            <div key={plan.id} className="ward-card anim-fade-up d1" style={{ marginBottom: 8 }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <div>
                  ...
                  <div style={{ fontSize: 19, fontWeight: 900, marginBottom: 2 }}>صفحات {plan.from} – {plan.to}</div>
                  <div style={{ fontSize: 11.5, opacity: 0.7 }}>{plan.to - plan.from + 1} صفحة</div>
                </div>
                <button onClick={() => onStartReview(plan.from, plan.to, plan.id)}
                  style={{ background: 'rgba(255,255,255,0.18)', border: '1px solid rgba(255,255,255,0.28)', color: 'white', padding: '9px 18px', borderRadius: 'var(--r-md)', fontFamily: "'Tajawal',sans-serif", fontWeight: 700, fontSize: 13.5, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 5 }}>
      ...
      {lastPos && (
        <div className="anim-fade-up d1" style={{ marginBottom: 14 }}>
          <button onClick={() => onStartReview(lastPos.page, Math.min(604, lastPos.page + 4))}
            style={{ width: '100%', padding: '12px 16px', borderRadius: 'var(--r-lg)', border: '1px solid var(--gold-border)', background: 'var(--gold-bg)', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 12, fontFamily: "'Tajawal',sans-serif" }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 10, marginBottom: 16 }} className="anim-fade-up d2">
        <div className="stat-card"><div className="stat-num" style={{ color: 'var(--ink)' }}>{stats.total}</div><div className="stat-lbl">إجمالي الأخطاء</div></div>
        <div className="stat-card"><div className="stat-num" style={{ color: 'var(--green)' }}>{stats.resolved}</div><div className="stat-lbl">تم حلها</div></div>
        <div className="stat-card"><div className="stat-num" style={{ color: 'var(--red2)' }}>{stats.pending}</div><div className="stat-lbl">معلّقة</div></div>
      </div>
      ...
      <button className="anim-fade-up d3" onClick={() => setShowModal(true)}
        style={{ width: '100%', padding: '18px 22px', borderRadius: 'var(--r-xl)', border: 'none', cursor: 'pointer', background: 'linear-gradient(135deg,var(--green) 0%,var(--green2) 60%,var(--green3) 100%)', color: 'white', display: 'flex', alignItems: 'center', gap: 14, boxShadow: '0 8px 28px var(--green-glow)', transition: 'all 0.2s', marginBottom: 14, fontFamily: "'Tajawal',sans-serif" }}>
      ...
      {recentSessions.length > 0 && (
        <div className="anim-fade-up d4">
          <div className="section-label" style={{ marginBottom: 8 }}>آخر الجلسات</div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 7 }}>
            {recentSessions.map((s, i) => (
              <div key={i} className="card-flat" style={{ padding: '11px 14px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <div>
                  <div style={{ fontSize: 13.5, fontWeight: 700, color: 'var(--ink)' }}>صفحات {s.fromPage} – {s.toPage}</div>
                  <div style={{ fontSize: 11, color: 'var(--ink4)', marginTop: 2 }}>{formatDate(s.date)}</div>
                </div>
                {s.mistakeCount > 0 ? <span className="badge badge-red">{s.mistakeCount} خطأ</span> : <span className="badge badge-green">نظيف ✓</span>}
              </div>
            ))}
          </div>
        </div>
            ...
            <div style={{ maxHeight: 250, overflowY: 'auto', marginBottom: 14 }}>
              {loading ? (
                <div style={{ textAlign: 'center', padding: 20, color: 'var(--ink4)' }}>
                  <Loader2 size={22} className="spin" /><div style={{ fontSize: 13, marginTop: 8 }}>جارٍ التحميل...</div>
                </div>
              ) : surahs.map(s => (
                <button key={s.id}
...
// ─── Review Page ──────────────────────────────────────────────────────────────
function ReviewPage({ from, to, taskId, settings, onFinish, onPlayAudio, playingKey, hasPlayer, onVersesLoaded }) {
  ...
  const handleVerseClick = (verse, e) => {
    ...
    const menuH = 200, spaceBelow = window.innerHeight - rect.bottom;
    ...
    <div style={{ minHeight: '100dvh', background: 'var(--bg)', display: 'flex', flexDirection: 'column' }}>
      <div className="review-header">
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 9 }}>
            <span style={{ fontSize: 10.5, color: 'var(--ink4)', fontWeight: 600 }}>{page - from + 1}/{total}</span>
            <div style={{ width: 80, height: 3, borderRadius: 999, background: 'var(--bg3)', overflow: 'hidden' }}>
              <div style={{ height: '100%', borderRadius: 999, background: 'var(--green)', width: `${progress}%`, transition: 'width 0.4s' }} />
            </div>
          </div>
          <div style={{ fontWeight: 800, fontSize: 14, color: 'var(--ink)' }}>صفحة {page}</div>
          <button onClick={handleFinish}
            style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '7px 12px', borderRadius: '10px', border: '1.5px solid var(--green-border)', background: 'var(--green-bg)', color: 'var(--green)', fontFamily: "'Tajawal',sans-serif", fontWeight: 800, fontSize: 12, cursor: 'pointer' }}>
      ...
      {menuPos && activeVerse && (
        <div ref={menuRef} className="verse-menu" style={{ top: menuPos.top, left: menuPos.left, right: 'auto', width: 215 }}>
          ...
          <button className="verse-menu-item" onMouseDown={e => { e.preventDefault(); e.stopPropagation(); handlePlayAudio(activeVerse); }}>
            <Volume2 size={14} color="var(--gold2)" /> استمع للآية
          </button>
          <button className="verse-menu-item" onMouseDown={e => {
      ...
      <div className="review-footer" style={{ bottom: hasPlayer ? 80 : 0 }}>
      ...
      {mistakeModalVerse && (
        <MistakeModal page={page} verseKey={mistakeModalVerse.verse_key} verseText={mistakeModalVerse.text_uthmani}
          onClose={() => setMistakeModalVerse(null)} onSaved={onMistakeSaved} />
      )}
      {tafsirModalVerse && (
        <TafsirModal verseKey={tafsirModalVerse.verse_key} verseText={tafsirModalVerse.text_uthmani}
          tafsirId={settings.defaultTafsir || '91'} onClose={() => setTafsirModalVerse(null)} />
      )}
    </div>
  );
}
  ...
  return (
    <div style={{ minHeight: '100dvh', background: 'var(--bg)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
      <div className="card anim-fade-up" style={{ padding: '36px 26px', maxWidth: 400, width: '100%', textAlign: 'center' }}>
        <div style={{ width: 76, height: 76, borderRadius: '20px', background: mistakes.length === 0 ? 'var(--green-bg)' : 'var(--gold-bg)', border: `2px solid ${mistakes.length === 0 ? 'var(--green-border)' : 'var(--gold-border)'}`, display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 20px' }}>
        ...
        {taskId && (
          <div style={{ padding: '10px 14px', borderRadius: 'var(--r-md)', background: 'var(--green-bg)', border: '1px solid var(--green-border)', marginBottom: 18, fontSize: 13, fontWeight: 700, color: 'var(--green)' }}>
            ✓ تم تحديد المهمة كمكتملة في الجدول
          </div>
        )}
        {mistakes.length > 0 && (
        ...
        <div className="sheet-body" style={{ paddingBottom: 20 }}>
          <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap', marginBottom: 14 }}>
            {TAFSIR_OPTIONS.map(t => (
              <button key={t.id} onClick={() => setSelectedId(t.id)}
                style={{ padding: '5px 11px', borderRadius: '999px', border: `1.5px solid ${selectedId === t.id ? 'var(--green)' : 'var(--border2)'}`, background: selectedId === t.id ? 'var(--green-bg)' : 'var(--surface2)', color: selectedId === t.id ? 'var(--green)' : 'var(--ink3)', fontSize: 12, fontWeight: 700, cursor: 'pointer', fontFamily: "'Tajawal',sans-serif" }}>
                {t.name}
              </button>
            ))}
          </div>
          ...
          {loading ? (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {[1, 2, 3, 4].map(i => <div key={i} className="skel" style={{ height: 16, opacity: 0.7 - i * 0.1 }} />)}
            </div>
          ) : error || !tafsir?.text ? (
            ...
            <div style={{ fontSize: 15, lineHeight: 2.4, color: 'var(--ink)', fontFamily: "'Noto Naskh Arabic','Scheherazade New',serif", direction: 'rtl', textAlign: 'justify', whiteSpace: 'pre-line' }}>
...
function MistakesScreen({ onFix, onCountChange }) {
  ...
  const load = useCallback(() => {
    const m = getMistakes(); setMistakes(m);
...
function FixMode({ mistake: init, settings, onBack, onDone }) {
  ...
  const handleFail = () => {
    ...
    refresh(); setResult('fail'); setShowVerse(false);
  };
          ...
          {Array.from({ length: daysInMonth }).map((_, i) => {
            ...
            return (
              <div key={d} className={cls}
                style={{ outline: isSelected ? '2px solid var(--green)' : 'none', outlineOffset: 1 }}
      ...
      <div style={{ marginBottom: 18 }}>
        <div className="divider" style={{ marginBottom: 12 }}>
          <span style={{ fontSize: 12.5, fontWeight: 700 }}>
            {displayDate === todayStr && !selected ? 'اليوم' : formatDate(displayDate)}
          </span>
        </div>
        {dayPlans.length > 0 ? (
              ...
              <div key={plan.id} className="card" style={{
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  <div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: 3 }}>
                      {plan.completed
                        ...
                        : <span style={{ fontSize: 12, fontWeight: 700, color: 'var(--green)' }}>🔄 مراجعة</span>}
                    </div>
                  ...
                  <div style={{ display: 'flex', gap: 7 }}>
                    {!plan.completed && (
                      <button className="btn btn-primary btn-sm" onClick={() => onStartReview(plan.from, plan.to, plan.id)}>ابدأ</button>
                    )}
                    ...
                    <button className="btn btn-icon" style={{ width: 32, height: 32 }} onClick={() => { deleteScheduleItem(plan.id); load(); }}>
                      <Trash2 size={14} />
                    </button>
                  </div>
                </div>
          ...
          {!showPreview ? (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <div>
                ...
                <input type="range" dir="ltr" min={1} max={60} step={1} value={days} onChange={e => setDays(Number(e.target.value))}
                  style={{ background: `linear-gradient(to left, var(--bg3) ${100 - ((days - 1) / 59) * 100}%, var(--green) ${100 - ((days - 1) / 59) * 100}%)` }} />
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 10.5, color: 'var(--ink4)', marginTop: 2 }}><span>1</span><span>60</span></div>
              </div>
            ...
            <>
              <div style={{ display: 'flex', gap: 10, marginBottom: 14 }}>
                <div className="stat-card" style={{ flex: 1, padding: '11px 10px' }}>
                  <div className="stat-num" style={{ color: 'var(--green)', fontSize: 22 }}>{preview.length}</div>
                  <div className="stat-lbl">أيام</div>
                </div>
                <div className="stat-card" style={{ flex: 1, padding: '11px 10px' }}>
                  <div className="stat-num" style={{ color: 'var(--gold2)', fontSize: 22 }}>{totalPages}</div>
                  <div className="stat-lbl">صفحة</div>
                </div>
                <div className="stat-card" style={{ flex: 1, padding: '11px 10px' }}>
                  <div className="stat-num" style={{ color: 'var(--ink)', fontSize: 22 }}>{pagesPerDay}</div>
...
function QuartersScreen({ onPlayAudio, settings, playingKey }) {
  ...
  const getRubMarkers = (vv) => {
    ...
    vv.forEach(v => {
      ...
      if (rubNum && rubNum !== lastRub) {
        ...
        let label = '';
        switch (rubInHizb) {
          case 1: label = `الحزب ${hizbNum} · الربع الأول`; break;
          case 2: label = `الحزب ${hizbNum} · النصف`; break;
          case 3: label = `الحزب ${hizbNum} · ثلاثة أرباع`; break;
          case 0: label = `نهاية الحزب ${hizbNum}`; break;
          default: label = `ربع ${rubNum}`;
        }
  ...
  const filtered = surahs.filter(s =>
    s.name_arabic?.includes(search) || s.name_simple?.toLowerCase().includes(search.toLowerCase()) || String(s.id).includes(search)
  );
    ...
    <div className="page-content">
      ...
      {loading ? (
        ...
        <div key={surah.id} style={{ marginBottom: 5 }}>
          <button
            style={{ width: '100%', padding: '13px 14px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: selected?.id === surah.id ? 'var(--green-bg)' : 'var(--surface)', border: `1px solid ${selected?.id === surah.id ? 'var(--green-border)' : 'var(--border)'}`, borderRadius: selected?.id === surah.id && verses.length > 0 ? 'var(--r-lg) var(--r-lg) 0 0' : 'var(--r-lg)', cursor: 'pointer', fontFamily: "'Tajawal',sans-serif", transition: 'all 0.15s' }}
            ...
            <div className="card" style={{ borderRadius: '0 0 var(--r-lg) var(--r-lg)', borderTop: 'none', padding: '14px' }}>
              {loadingV ? (
                ...
                <>
                  ...
                  {markers.map((m, i) => (
                    <div key={i} style={{
                      padding: '11px 13px', borderRadius: 'var(--r-md)', marginBottom: 7,
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        <div className="quran-text" style={{ fontSize: 15, lineHeight: 1.9, color: 'var(--ink)' }}>
                          {m.text_uthmani?.slice(0, 80)}{m.text_uthmani?.length > 80 ? '…' : ''}
                        </div>
                      </div>
                      <button className="btn btn-ghost btn-sm" style={{ flexShrink: 0, padding: '5px 9px' }}
                        onClick={() => { if (!m.verse_key) return; const [s,a] = m.verse_key.split(':'); onPlayAudio(s, a, m.verse_key, m.text_uthmani); }}>
          ...
          <div style={{ display: 'flex', gap: 5 }}>
            {[{ id: 'sm', l: 'ص', size: 14 }, { id: 'md', l: 'م', size: 18 }, { id: 'lg', l: 'ك', size: 22 }].map(s => (
              <button key={s.id} onClick={() => onChange({ quranFontSize: s.id })}
                style={{ width: 34, height: 34, borderRadius: '9px', border: `1.5px solid ${settings.quranFontSize === s.id ? 'var(--green)' : 'var(--border2)'}`, background: settings.quranFontSize === s.id ? 'var(--green-bg)' : 'transparent', color: settings.quranFontSize === s.id ? 'var(--green)' : 'var(--ink3)', fontWeight: 700, fontSize: s.size - 2, cursor: 'pointer', fontFamily: "'Scheherazade New',serif" }}>
                {s.l}
              </button>
            ))}
          </div>
